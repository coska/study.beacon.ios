...
class BeaconAPI: NSObject {
    ...
    // MARK: - Privates
    var supportedBeaconInfo: [String:String] {
    ...
    var beaconUUIDs: [String] {
        return Array(supportedBeaconInfo.keys)
    }
    ...
    func startMonitoringForRegion(forBeacon: Beacon) {
        ...
        let region = CLBeaconRegion(proximityUUID: uuid, identifier: forBeacon.name)
...
//            locationManager.startMonitoringForRegion(region)
            ...
            for strUUID in supportedBeaconInfo.keys {
                guard let uuid = NSUUID(UUIDString: strUUID) else { return }
                var beaconName: String = ""
                if let name = supportedBeaconInfo[strUUID] {
                    beaconName = name
                }
                
                let region = CLBeaconRegion(proximityUUID: uuid, identifier: beaconName)
//                locationManager.startMonitoringForRegion(region)
                locationManager.startRangingBeaconsInRegion(region)
            }
        }
